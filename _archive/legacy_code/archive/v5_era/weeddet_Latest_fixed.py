...
Architecture:
  Input RGB image (512×512, letterboxed — aspect ratio preserved with grey padding)
      │
...
Training:
  ...
  Linear warmup 300 iters  →  CosineAnnealingLR
  Freeze BatchNorm (batch_size=2 too small for stable BN stats)
  Grad clip 0.5  (stability insurance)

Preprocessing (train & inference):
  Letterbox: resize-with-aspect-ratio, pad to 512×512 with grey (114,114,114).
  Boxes are scaled into letterbox space during training and unpadded at inference.
  This replaces the previous squish-resize which caused coordinate artifacts on
  non-square images (bottom-edge box clustering).
...
# LETTERBOX UTILITIES
# ===========================================================================
# Used in both WeedDataset (training) and predict() (inference) so that the
# exact same spatial transformation is applied in both paths.
# Boxes predicted in letterbox space are remapped to original coords at infer.

def letterbox_pil(img_pil, target=512):
    """
    Resize a PIL image preserving aspect ratio, then pad to target×target
    with grey (114,114,114).  Returns (padded_img, scale, pad_left, pad_top).
    """
    ...
    nw     = round(w * scale)
    nh     = round(h * scale)
    resized = img_pil.resize((nw, nh), _PIL.BILINEAR)
    pad_l   = (target - nw) // 2
    pad_t   = (target - nh) // 2
    canvas  = _PIL.new('RGB', (target, target), (114, 114, 114))
    canvas.paste(resized, (pad_l, pad_t))
    return canvas, scale, pad_l, pad_t


def scale_boxes_to_letterbox(boxes, scale, pad_l, pad_t):
    """Scale xyxy boxes (original space) into letterbox space."""
...
def unpad_boxes(boxes, scale, pad_l, pad_t):
    """Map xyxy boxes from letterbox space back to original image space."""
    if boxes is None or len(boxes) == 0:
        return boxes
    b = boxes.clone().float()
    b[:, [0, 2]] = (b[:, [0, 2]] - pad_l) / scale
    b[:, [1, 3]] = (b[:, [1, 3]] - pad_t) / scale
    return b
...
class WeedDet(nn.Module):
    ...
    def __init__(self, num_classes=1, anchor_base_scale=6, lsc_k=7):
        super().__init__()
        self.num_classes = num_classes
    ...
    def _get_logits(self, images):
        """Run backbone + FPN + head + anchor gen; return raw logits for _decode."""
    ...
    def _decode(self, cls_logits, regs, anchors, img_shape,
                score_thr=0.05, nms_thr=0.25, max_dets=300,
                output_thr=0.5):
        """
        score_thr  : pre-NMS candidate gate (keep low to feed NMS well)
        nms_thr    : IoU threshold for NMS — 0.25 is aggressive, cleans dense
                     rice-paddy over-detections while keeping one box/plant
        max_dets   : hard cap after NMS
        output_thr : post-NMS confidence gate (default 0.5).
                     VariFocal scores ≈ IoU, so 0.5 means "I predict ≥50% IoU".
                     Lower to 0.35 if you want more recall; raise to 0.6 for fewer.
        """
        ...
        for b in range(B):
            ...
            # Pre-NMS gate: low threshold to give NMS all viable candidates
            ...
            # Aggressive NMS: 0.25 collapses duplicate boxes on the same plant
            keep2  = nms(boxes, scores, nms_thr)[:max_dets]
            boxes  = boxes[keep2]
            scores = scores[keep2]
            labels = labels[keep2]

            # Post-NMS output gate: filters lukewarm background candidates
            keep3  = scores >= output_thr
            results.append({
                'boxes' : boxes[keep3],
                'scores': scores[keep3],
                'labels': labels[keep3],
...
class WeedDataset(Dataset):
    ...
    def __init__(self, root, split='train',
        ...
        self.root       = root
        self.target_size = img_size if isinstance(img_size, int) else max(img_size)
        ...
        # No T.Resize — letterbox handles spatial resizing in __getitem__
    ...
    def _augment(self, img, boxes):
        """
        Augmentations for rice paddy training.

        Changes from original:
          - GaussianBlur ceiling 1.0 → 0.5  (was destroying fine leaf texture)
          - equalize probability 0.5 → 0.15  (equalization nukes the green-soil
            colour contrast that separates rice from background)
          - Added colour jitter (saturation + hue) — encodes lighting variation
            without destroying structural texture
        Kept as-is:
          - horizontal flip (50 %)       — paddies are symmetric
          - translation ±10 %  (50 %)   — simulates camera framing variation
          - brightness ±30 %   (50 %)   — handles sun / shadow
        """
        import random
        ...
        # Colour saturation ±40 %  (new — handles water reflection variation)
        if random.random() > 0.5:
            img = ImageEnhance.Color(img).enhance(
                random.uniform(0.6, 1.4))

        # Mild blur — ceiling lowered 1.0 → 0.5 to preserve leaf edge texture
        if random.random() > 0.4:
            img = img.filter(
                ImageFilter.GaussianBlur(random.uniform(0, 0.5)))

        # Equalize — probability lowered 50 % → 15 % to preserve colour cues
    ...
    def __getitem__(self, idx):
        ...
        # Augment on the original (un-letterboxed) image first
        if self.augment and len(boxes):
            img, boxes = self._augment(img, boxes)

        # Letterbox: preserve aspect ratio, pad to target_size × target_size.
        # This replaces the previous img.resize() which squished non-square
        # images and caused coordinate artifacts (bottom-edge box clustering).
        img, scale, pad_l, pad_t = letterbox_pil(img, self.target_size)
        if len(boxes):
            boxes = scale_boxes_to_letterbox(boxes, scale, pad_l, pad_t)
...
def train(config):
    ...
    train_ds = WeedDataset(config['data_root'], 'train', img_size, augment=config.get('augment', True))
    ...
    if scheduler_type == 'cosine':
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=config.get('num_epochs', 60),
            eta_min=config.get('min_lr', config.get('base_lr', 0.001) * 0.05))
    else:
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer,
            milestones=config.get('lr_decay_epochs', [40, 55]),
            gamma=0.1)

    warmup   = WarmupMultiStepLR(optimizer,
                                 warmup_iters=config.get('warmup_iters', 300),
...
def train_with_progress(config):
    ...
    img_size = config.get('img_size', 512)
    train_ds = WeedDataset(config['data_root'], 'train', img_size,
...
def predict(model, image_path, img_size=512,
            score_thr=0.5, nms_thr=0.25, device='cuda'):
    """
    Run WeedDet on a single image.  Returns list of detection dicts with boxes
    in the ORIGINAL image's coordinate space (not the letterbox space).

    img_size  : letterbox target side length (default 512, must match training)
    score_thr : post-NMS confidence gate (VariFocal score ≈ predicted IoU)
    nms_thr   : IoU threshold for NMS (0.25 aggressively collapses duplicates)
    """
    from PIL import Image as PILImage
    img      = PILImage.open(image_path).convert('RGB')

    # --- Letterbox (same transform as training) ---
    img_lb, scale, pad_l, pad_t = letterbox_pil(img, img_size)

    tf = T.Compose([
        T.ToTensor(),
        T.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD),
    ])
    x = tf(img_lb).unsqueeze(0).to(device)

    with torch.no_grad():
        # Pass custom decode thresholds — score_thr low for NMS, output_thr = score_thr
    ...
    # --- Unpad: map boxes back to original image coordinates ---
    boxes_orig = unpad_boxes(preds['boxes'].cpu(), scale, pad_l, pad_t)

    dets = []
    for i in range(len(boxes_orig)):
        dets.append({
            'box'  : boxes_orig[i].tolist(),   # [x1, y1, x2, y2] original pixels
...
if __name__ == '__main__':
    ...
    tgts = [
        {'boxes' : torch.tensor([[50,50,200,250]], dtype=torch.float32).to(device),
         'labels': torch.tensor([0], dtype=torch.int64).to(device),
         'orig_h': 512, 'orig_w': 512},
    ...
    with torch.no_grad():
        ...
        for _ in range(30):
            model(x)
        fps = 30 / (time.time() - t0)
    print(f"Inference FPS @ 512×512: {fps:.1f}")

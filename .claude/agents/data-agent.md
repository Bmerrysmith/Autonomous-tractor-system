...
model: opus
description: >
  Use this agent to audit AgriNav datasets before they're trusted for
  training or evaluation: verifying the detector/RiceSEG split is actually
  leak-free, estimating label-noise or missing-annotation rates, checking
  class distribution and domain match against deployment, or vetting a new
  candidate external dataset before it's allowed near
  data/manifests/detector_split_v1.json. This is the last line of defense
  between a dataset and the model trained on it — invoke it before any
  dataset is declared usable, not after a bad result shows up in training.

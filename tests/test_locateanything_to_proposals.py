...
from scripts.locateanything_to_proposals import (
    DEFAULT_PROMPT_MAP,
    ProposalError,
    build_prompt_map,
    build_proposal_doc,
...
class BuildProposalDoc(unittest.TestCase):
    def test_unmapped_prompt_is_a_counted_drop_not_rice(self):
        unit = _unit(detections=[
            {"prompt": "some plant", "bbox": [1, 1, 5, 5], "score": 0.99},
        ])
        doc, stats = build_proposal_doc(
            image_units=[unit], prompt_map=dict(DEFAULT_PROMPT_MAP),
            model_id="m", model_revision="deadbeef", score_threshold=0.3,
        )
    ...
    def test_score_and_oob_boxes_are_dropped_and_counted(self):
        unit = _unit(detections=[
            {"prompt": "cultivated rice plants", "bbox": [10, 10, 10, 10], "score": 0.1},   # low score
            {"prompt": "weeds growing among the rice plants", "bbox": [200, 200, 5, 5], "score": 0.9},  # oob
            {"prompt": "cultivated rice plants", "bbox": [10, 10, 20, 20], "score": 0.9},   # kept
        ])
        _doc, stats = build_proposal_doc(
            image_units=[unit], prompt_map=dict(DEFAULT_PROMPT_MAP),
            model_id="m", model_revision="deadbeef", score_threshold=0.5,
        )
    ...
    def test_every_annotation_is_an_unreviewed_proposal_with_provenance(self):
        ...
        doc, _stats = build_proposal_doc(
            image_units=[unit], prompt_map=dict(DEFAULT_PROMPT_MAP),
            model_id="nvidia/LocateAnything-3B", model_revision="deadbeef",
            score_threshold=0.3, generated_at="2026-07-21T00:00:00Z",
        )
    ...
    def test_output_indexes_in_sam_step_without_normalising_to_rice(self):
        unit = _unit(detections=[
            ...
            {"prompt": "ambiguous vegetation that may not be rice", "bbox": [5, 5, 20, 20], "score": 0.9},
        ])
        doc, _stats = build_proposal_doc(
            image_units=[unit], prompt_map=dict(DEFAULT_PROMPT_MAP),
            model_id="m", model_revision="deadbeef", score_threshold=0.3,

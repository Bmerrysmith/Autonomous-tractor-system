...
from baseline_seg_control import ARCHS, build_model, stability_stats  # noqa: E402
...
def _hist(mious, weeds=None, n_from=1):
    ...
    for i, m in enumerate(mious):
        ...
        out.append({
            ...
            'loss': 1.0,
            'miou': m,
            'per_class_iou': {'background': 0.9, 'green_veg': 0.9,
                              'senescent': 0.3, 'panicle': 0.7,
                              'weeds': w, 'duckweed': 0.3},
            'absent_classes': [],
        })
    return out
...
def test_every_class_is_reported_even_when_never_present():
    ...
    assert set(s['per_class']) == {
        'background', 'green_veg', 'senescent', 'panicle', 'weeds', 'duckweed'}
